...
def predict_future(embeddings, target_year, context_data=None):
    ...
    for epoch in range(epochs):
        ...
        rmse = compute_rmse(predictions, train_y)
        mae = compute_mae(predictions, train_y) 
    ...
    with torch.no_grad():
        context = torch.stack(context_data)
        final_predictions = model(train_x, context)
        final_predictions = final_predictions.reshape(num_samples, num_nodes, features)
    
        final_rmse = compute_rmse(final_predictions, train_y)
        final_mae = compute_mae(final_predictions, train_y)

        print(f"\nFinal Training RMSE: {final_rmse:.4f}")
